#!/bin/bash
# Benchmark: Covering vs non-covering indexes on MariaDB
...
output_csv="$SCRIPT_DIR/../results/mariadb_covering_index.csv"  # change if needed
...
bash "$SCRIPT_DIR/../configure_mariadb.sh" \
  "$database_name" \
  "DROP INDEX IF EXISTS nc4 ON employee;
...
echo "MariaDB covering index benchmark finished. Results saved to: $output_csv"

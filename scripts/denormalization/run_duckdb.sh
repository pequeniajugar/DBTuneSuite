#!/bin/bash
# Benchmark: Denormalization on DuckDB (big TPCH) using base_duckdb.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

duckdb_db="path to/db name"  # change if needed
output_csv="$SCRIPT_DIR/../results/duckdb_big_denormalization.csv"  # change if needed
...
#duckdb "$duckdb_db" -c "DROP TABLE IF EXISTS lineitemdenormalized;"

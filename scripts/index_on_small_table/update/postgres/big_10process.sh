...
RESULTS_CSV = RESULTS_DIR / "postgres_index_small_update_10process.csv"
...
def update_task(process_id, index_type, updates, result_queue):
    ...
    # Postgres does not support FORCE INDEX; we rely on the planner to use the PK / secondary index
    if index_type == "clustered":

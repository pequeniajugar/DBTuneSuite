...
RESULTS_CSV = RESULTS_DIR / "postgres_index_small_update_2process.csv"
...
def update_task(process_id, index_type, updates, result_queue):
    ...
    # Postgres has no FORCE INDEX, so we just write normal UPDATEs and rely on the planner.
    if index_type == "clustered":
        sql_template = "UPDATE employees SET name = %s WHERE ssnum = %s"
    elif index_type == "nonclustered":
...
def run_experiment(index_type, run_number):
    ...
    with RESULTS_CSV.open("a", newline="") as f:
        ...
        writer.writerow([
            "postgres",        # dbms label
            index_type,        # label: scan / nonclustered / clustered
            run_number,
            f"{execution_time:.6f}",
            f"{response_time:.6f}",
        ])

...
POSTGRES_DB_PARAMS = {
    ...
    "dbname": "employees_small",  # change DB name if needed
}
...
RESULTS_CSV = RESULTS_DIR / "postgres_index_small_search_2process.csv"
...
def update_task(process_id, use_index, updates, result_queue):
    ...
    for new_name, value in updates:
        try:
            ...
            # SELECT queries don't need commit, just count them
            updates_count += 1
        except psycopg2.Error as e:
...
def run_experiment(use_index, run_number):
    print(f"\n[Run {run_number}/10] Running experiment with {'INDEX' if use_index else 'NO INDEX'} (Postgres)...")

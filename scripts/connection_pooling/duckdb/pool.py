...
def parse_args():
    ap = argparse.ArgumentParser(description="DuckDB pooled insert benchmark (fake pool)")
    ap.add_argument("--db", required=True, help="Path to DuckDB database file")
    ap.add_argument("--pool-size", type=int, required=True, help="(Unused, for compatibility)")
...
def run_experiment(query, exec_times, resp_times, db_path):
    attempt = 0
    while attempt < 2000:
        try:
...
def main():
    ...
    for i in range(args.threads):
        t = threading.Thread(target=run_experiment, args=(queries[i+1], execs, resps, db_path))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
    ...
    print(f"POOL | Threads={args.threads} | Pool={args.pool_size}")
    ...
    write_result_csv(db_path, "pool", args.pool_size, args.threads, total_cpu, total_wall)

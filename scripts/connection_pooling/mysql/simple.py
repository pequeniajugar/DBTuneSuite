# === simple.py (MySQL with socket/TCP, NullPool) ===
import time
import threading
import random
import argparse
...
def parse_args():
    ap = argparse.ArgumentParser(description="MySQL simple (no pool) insert benchmark")
...
def make_db_url(args):
    pw = quote_plus(args.password or "")
    if args.socket:
        return f"mysql+pymysql://{args.user}:{pw}@/{args.db}?unix_socket={args.socket}"
    else:
        return f"mysql+pymysql://{args.user}:{pw}@{args.host}:{args.port}/{args.db}"
...
def set_mysql_max_connections(db_url, val):
...
def main():
    ...
    set_mysql_max_connections(db_url, args.max_conn)
    with get_raw_connection(db_url) as conn:

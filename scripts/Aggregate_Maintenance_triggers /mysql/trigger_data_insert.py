...
# Make sure this matches however many columns you actually use
# in the CSV for (ordernum, itemnum, quantity, storeid, vendorid).
# If it's defined elsewhere in your real file, keep that instead.
COLUMN_COUNT = 5


def get_connection():
    """Create a new MySQL connection using MYSQL_CONFIG."""
...
def append_insert_result(csv_path: Path, label: str, iteration: int, exec_time: float, resp_time: float):
    with csv_path.open("a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["mysql", label, iteration, f"{exec_time:.6f}", f"{resp_time:.6f}"])
...
def run_insert_experiment(tbl_file_path: str, mode: str):
    ...
    - mode == "with_trigger": clear orders between runs 1-10,
    ...
    Insertion timings are written into results/mysql_aggregate_triggers_insert.csv
    with labels "with_trigger_insert" or "without_trigger_insert".
    """
    # go up two levels from this script:
    #   scripts/dbms/mysql/trigger_data_insert.py
    # → scripts/
    root_dir = Path(__file__).resolve().parents[2]
    results_csv = root_dir / "results" / "mysql_aggregate_triggers_insert.csv"
    ...
    print(f"\n=== Row-by-row INSERT experiment ({mode}, MySQL) ===")
...
def main():
    parser = argparse.ArgumentParser(
        description="Row-by-row INSERT experiment for Aggregate Maintenance (MySQL)."
    )
    parser.add_argument(
        "--mode",

...
POSTGRES_CONFIG = {
    ...
    "dbname": "store_new_10_5",
}

# Default path to the CSV file with order rows (adjust if needed)
DEFAULT_TBL_FILE_PATH = "/data/tw3090/store/triggers_input.csv"

# Number of columns taken from CSV for (ordernum, itemnum, quantity, storeid, vendorid)
COLUMN_COUNT = 5


def get_connection():
    """Create a new PostgreSQL connection using POSTGRES_CONFIG."""
...
def run_insert_experiment(tbl_file_path: str, mode: str):
    ...
    Insertion timings are written into results/postgres_aggregate_triggers_insert.csv
    with labels "with_trigger_insert" or "without_trigger_insert".
    """
    # go up two levels from this script:
    #   scripts/dbms/postgres/trigger_data_insert.py
    # → scripts/
...
def main():
    parser = argparse.ArgumentParser(
        description="Row-by-row INSERT experiment for Aggregate Maintenance (PostgreSQL)."
    )
    parser.add_argument(
        "--mode",

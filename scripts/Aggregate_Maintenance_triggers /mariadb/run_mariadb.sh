#!/bin/bash
# Benchmark: Aggregate Maintenance with and without Triggers on MariaDB
...
database_name="store_new_10_5"  # change if needed (should match MARIADB_CONFIG['database'] in trigger_data_insert.py)
tbl_file_path="/data/tw3090/store/triggers_input.csv"  # change if needed

# CSV for query timings (all base_mariadb.sh runs)
query_output_csv="$SCRIPT_DIR/../results/mariadb_aggregate_triggers_queries.csv"  # change if needed

# Ensure results directory exists for this path
mkdir -p "$(dirname "$query_output_csv")"
# Start fresh so base_mariadb.sh writes the header once
rm -f "$query_output_csv"

# --- Capture initial max ordernum before all experiments (for restoration) ---
MYSQL_USER="root"        # keep in sync with scripts/configure_mariadb.sh
MYSQL_PASSWORD="pwd"
MYSQL_HOST="localhost"
MYSQL_PORT=15559

initial_max_ordernum=$(mysql -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" \
  -h "$MYSQL_HOST" -P "$MYSQL_PORT" -N -D "$database_name" \
  -e "SELECT COALESCE(MAX(ordernum), 0) FROM orders;" || echo 0)

echo "=== Aggregate Maintenance experiment on MariaDB ==="
...
echo ">>> Measuring query performance WITH triggers using base_mariadb.sh..."
...
echo ">>> Measuring query performance WITHOUT triggers using base_mariadb.sh..."

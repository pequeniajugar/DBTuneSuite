#!/bin/bash
# Benchmark: Hash vs B+Tree vs No Index on MariaDB (Point Query on ssnum)
...
#change file name to mariadb_small_hash_vs_btree_point.csv
output_csv="$SCRIPT_DIR/../results/mariadb_big_hash_vs_btree_point.csv"
...
bash "$SCRIPT_DIR/../base_mariadb.sh" \
  "$database_name" \
  "SELECT * FROM employees FORCE INDEX (idx_ssnum) WHERE ssnum = {v1};" \
  "hash_index" \
  "$(basename "$output_csv")"

bash "$SCRIPT_DIR/../configure_mariadb.sh" "$database_name" \
  "DROP INDEX IF EXISTS idx_ssnum;"
...
# Restore InnoDB and reset memory sizes
bash "$SCRIPT_DIR/../configure_mariadb.sh" "$database_name" \

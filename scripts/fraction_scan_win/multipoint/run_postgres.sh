...
output_csv="$SCRIPT_DIR/../../results/postgres_fraction_scan_win_multipoint.csv"  # change if needed
...
run_fraction() {
  ...
  for val in "${values[@]}"; do
    # Non-clustered index query (disable seqscan)
    bash "$SCRIPT_DIR/../../base_postgres.sh" \
      "$database_name" \
      "SET enable_seqscan = off; SELECT * FROM scanwin_multipoint WHERE ${column_indexed} = ${val};" \

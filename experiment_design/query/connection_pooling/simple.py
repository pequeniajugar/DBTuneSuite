...
def run_experiment(thread_id, query, exec_times, resp_times):
    attempt = 0
    while attempt < 2000:
        try:
...
def run():
    ...
    for i in range(NUM_THREADS):
        t = threading.Thread(target=run_experiment, args=(i + 1, queries[i + 1], execs, resps))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
    ...
    print(f"SIMPLE | Threads={NUM_THREADS} | max_conn={MAX_CONN}")
